...
for error_rate in [0, 0.00001, 0.0001, 0.001, 0.01, 0.1]:
    ...
    for layout_n in [1, 2, 4, 8]:
        ...
        for j in range(try_times):
            ...
            guess = 0
            min_dis=99999
            for i in range(len(bench_total_power_traces)):
                l = min(len(new_power_trace), len(bench_total_power_traces[i]))
                t_dis = 0
                for j in range(l):
                    t_dis +=abs(bench_total_power_traces[i][j] - new_power_trace[j])
                if t_dis < min_dis:
                    min_dis=t_dis
                    guess=i
            if guess == ans:
